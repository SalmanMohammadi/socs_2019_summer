...
def get_dataset(
    ...
    def _tf_midi_to_note_sequence(ex):
      if config.include_style_labels:
        return tf.py_function(
              lambda x, y: [mm.midi_to_note_sequence(x.numpy()).SerializeToString(), y.numpy()],
              inp=[ex['midi'], ex['style']['primary']],
              Tout=[tf.string, tf.int64],
              name='midi_to_note_sequence_style')
      else:
        return tf.py_function(
  ...
  if config.include_style_labels:
    dataset = (dataset
               .map(lambda x, y: data_converter.tf_style_to_tensors(x, y, 
                data_converter.tf_to_tensors),
                    num_parallel_calls=tf.data.experimental.AUTOTUNE)
               .flat_map(lambda t, y: tf.data.Dataset.from_tensor_slices((t)).map(
                        lambda a, b, c, d: (a, b, c, d, y)))
               .map(lambda a, b, c, d, y: (_remove_pad_fn(a, b, c, d), y),
                    num_parallel_calls=tf.data.experimental.AUTOTUNE))
               # .map(lambda x, y: (*x, y))
               # .map(lambda a, b ,c ,d, e: tf.logging.info(e)))
  ...
  tf.logging.info(dataset.output_shapes)
...
class AugmentedGrooveConverter(BaseNoteSequenceConverter):
  """Converts to and from hit/velocity/offset representations.
  ...
  def __init__(self, split_bars=None, steps_per_quarter=4, quarters_per_bar=4,
               ...
               num_offset_bins=None, split_instruments=False, hop_size=None,
               hits_as_controls=False, fixed_velocities=False,
               max_note_dropout_probability=None, style_categories=None):

    self._split_bars = split_bars
    self._steps_per_quarter = steps_per_quarter
    self._steps_per_bar = steps_per_quarter * quarters_per_bar
    self._style_categories = style_categories
    ...
    super(AugmentedGrooveConverter, self).__init__(
        input_depth=output_depth,
        input_dtype=np.float32,
        output_depth=output_depth,
        output_dtype=np.float32,
  ...
  def tf_style_to_tensors(self, x, y, x_converter):
    """ Converts tf.float64 style labels to tensors.
    """
    inputs, outputs, controls, lengths = x_converter(x)
    style_vectors = tf.one_hot(y, self._style_categories)
    style_vectors.set_shape([self._style_categories])
    return (inputs, outputs, controls, lengths), style_vectors
